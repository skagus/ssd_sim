...
void DumpState(RV32IMACtx* gstCoreRegs, uint8_t* ram_image)
{
...
restart:
	{
		...
		if( dtb_file_name )
		{
			...
			else
			{
				...
				long dtblen = ftell( f );
				fseek( f, 0, SEEK_SET );
				dtb_ptr = gnSizeRAM - dtblen - sizeof(RV32IMACtx );
		...
		{
			// Load a default dtb.
			dtb_ptr = gnSizeRAM - sizeof(default64mbdtb) - sizeof(RV32IMACtx );
	...
	gstCoreRegs = (struct RV32IMACtx *)(ram_image + gnSizeRAM - sizeof(RV32IMACtx ));
	...
	for(uint64_t rt = 0; rt < instct+1 || instct < 0; rt += instrs_per_flip )
	{
		...
		int ret = CPU_Step( gstCoreRegs, ram_image, 0, elapsedUs, instrs_per_flip ); // Execute upto 1024 cycles before breaking out.
		switch( ret )
		{
			case 0:
				break;
